...
class TextileConverterTest {
    ...
    void emailAtSignNotCorrupted() {
        ...
        String input = "담당자: sample1@sample.org, sample2@sample.org";
        String result = converter.convert(input).trim();
        assertTrue(result.contains("sample1@sample.org"), "이메일 주소가 보존되어야 함");
        assertTrue(result.contains("sample2@sample.org"), "두 번째 이메일도 보존되어야 함");
    }
    ...
    void nestedListItemWithHyphenNotStrikethrough() {
        ...
        String result = converter.convert("** 담당자 : XX CO - 홍길동 수석(010-9999-9999)").stripTrailing();
        assertFalse(result.contains("~~"), "리스트 항목이 취소선으로 변환되면 안 됨");
        assertTrue(result.contains("XX CO - 홍길동"), "내용 중 하이픈이 보존되어야 함");
    }
    ...
    void table_textileHeadingPrefixOnHeaderRow_stripped() {
        ...
        assertEquals("| --- | --- |", lines[1]);
        assertEquals("| 샘플회사 | 시스템 개발 |", lines[2]);
    }
    ...
    void table_multilineCellContent_mergedWithBr() {
        ...
        assertTrue(result.contains("회계-28090(WEB)"), "연속 텍스트가 보존되어야 함");
    ...
    void table_continuationWithTrailingPipes_cleaned() {
        ...
        assertTrue(result.contains("회계-28090"), "연속 텍스트 내용은 유지되어야 함");
    }
}
