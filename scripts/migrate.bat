...
java -Dfile.encoding=UTF-8 -Dstdout.encoding=UTF-8 -Dstderr.encoding=UTF-8 -jar "%JAR%" migrate %* 2>&1 | powershell -noprofile -Command "[Console]::InputEncoding=[Console]::OutputEncoding=[System.Text.UTF8Encoding]::new($false);$enc=[System.Text.UTF8Encoding]::new($false);$sw=[System.IO.StreamWriter]::new($env:LOG,$true,$enc);try{$input|%%{[Console]::WriteLine($_);$sw.WriteLine($_)}}finally{$sw.Close()}"
exit /b %errorlevel%
