...
| 일감 (Issue) | `issues/{id}.md` + `issues.md` 인덱스 — 리포지터리 push (기본) |
| 일감 (Issue) | GitHub Issue (Label + Milestone + Assignee) (`REDMINE_ISSUE_MD_FETCH=false` 시) |
| 일감 댓글 / 변경 이력 | 일감 MD 파일의 Comments 섹션 (노트 + 필드 변경 이력) |
| 버전 (Version) | Milestone |
| 트래커 / 우선순위 / 상태 / 카테고리 | Label |
| Wiki 첨부파일 | 리포지터리 push (`attachments/`) |
| 이슈 첨부파일 | 리포지터리 push (`attachments-issue/`) |
...
│       ├── issues-json/           # Issue JSON + Label/Milestone 정의
│       ├── issues/                # Issue MD 파일 (REDMINE_ISSUE_MD_FETCH=true 시)
│       ├── issues.md              # 이슈 목록 인덱스 테이블
│       └── _migration/
...
- [사용자 매뉴얼](documents/USER_MANUAL.md)
- [GitHub 업로드 방식 비교](documents/GITHUB_UPLOAD.md)
